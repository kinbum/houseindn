...
class KindListDataTable extends AbstractDataTables
{
    protected $model;

    public function __construct()
    {
        $this->model = Kind::select(['id', 'name', 'description']);
    }
    ...
    public function run()
    {
        $this->setAjaxUrl(route('kinds.index.post'), 'POST');
    ...
    protected function fetchDataForAjax()
    {
        return datatable()->of($this->model)
            ...
            ->addColumn('actions', function ($item) {
                /*Edit link*/
                $deleteLink = route('kinds.delete.delete', ['id' => $item->id]);

                /*Buttons*/
                $editBtn = link_to(route('kinds.edit.get', ['id' => $item->id]), trans('webed-core::datatables.edit'), ['class' => 'btn btn-sm btn-outline green']);
